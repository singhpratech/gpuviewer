...
///
/// `force_mock` always includes the mock backend; otherwise the mock is the fallback when
/// no real backend initialized (so the TUI always has something to show, clearly labeled).
pub fn all_backends(force_mock: bool) -> Vec<Box<dyn GpuBackend>> {
    let mut backends: Vec<Box<dyn GpuBackend>> = Vec::new();

    // TODO(v1): nvidia (nvml-wrapper, lib_path("libnvidia-ml.so.1"))
    // TODO(v1): amd (sysfs/hwmon/gpu_metrics/fdinfo, root-dir parameterized)
    // TODO(v1): intel (fdinfo i915 + xe dialects, sysfs freq)

    if force_mock || backends.is_empty() {
